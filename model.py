...
class TranscriptEncoder(nn.Module):
    ...
    def forward(self, q, q_len):
        ...
        packed = pack_padded_sequence(q_embed, lengths=q_len.cpu(), batch_first=True)
...
class Classifier(nn.Module):
    ...
    def forward(self, bimodal_emb, config):
        # Tile ques_vector, concatenate
        ...
        preds_con = self.relu(preds) 
            #pdb.set_trace()
        
        return preds, preds_con
...
class BasePReFIL(nn.Module):
    def __init__(self, num_tokens, config):
        ...
        self.config = config
        self.rnn = TranscriptEncoder(num_tokens, config)
...
class PReFIL(BasePReFIL):
    ...
    def forward(self, img, ques, q_len, config):
        ...
        answer = self.classifier(final_feat, config)
        return answer
